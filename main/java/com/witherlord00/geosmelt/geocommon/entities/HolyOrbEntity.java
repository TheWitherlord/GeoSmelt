...
public class HolyOrbEntity extends AbstractFireballEntity {

	public int ticksInAir;

	public HolyOrbEntity(EntityType t, World worldIn) {
		super(t, worldIn);
	}

	public HolyOrbEntity(FMLPlayMessages.SpawnEntity spawnEntity, World worldIn) {
		this(GeoEntityRegistry.HOLY_ORB, worldIn);
	}

	public HolyOrbEntity(EntityType t, World worldIn, double posX, double posY, double posZ, double accelX,
	...
	public HolyOrbEntity(EntityType t, World worldIn, PlayerEntity shooter, double accelX, double accelY,
			double accelZ) {
	...
	protected void onImpact(RayTraceResult result) {
		...
		if (result.getType() != RayTraceResult.Type.ENTITY
			...
			if (!this.world.isRemote) {
				...
				areaeffectcloudentity.setParticleData(ParticleTypes.END_ROD);
				areaeffectcloudentity.setRadius(2.0F);
				areaeffectcloudentity.setDuration(90);
				areaeffectcloudentity.setRadiusPerTick(
						(4.0F - areaeffectcloudentity.getRadius()) / (float) areaeffectcloudentity.getDuration());
				areaeffectcloudentity.addEffect(new EffectInstance(Effects.REGENERATION, 25, 0));
				areaeffectcloudentity.addEffect(new EffectInstance(Effects.INSTANT_HEALTH, 25, 0));
				if (!list.isEmpty()) {
